...
#define INDENT_STEP 4 // Número de espaços por nível de indentação
...
void print_ast_node(ast_node_t *node, int indent_level) {
  ...
  // Imprime o nome do tipo de nó
  printf("%s", get_node_type_name(node->type));

  // Abre parênteses
  printf(" (");

  // Dependendo do tipo de nó, imprime seus filhos
  switch (node->type) {
  case AST_PROGRAM:
    printf("\n");
  ...
  case AST_VAR_DECLARATION:
    printf("\n");
    // Imprime o tipo e o identificador
  ...
  case AST_FUN_DECLARATION:
    printf("\n");
    // Imprime o tipo, identificador, parâmetros e corpo
    print_indent(indent_level + 1);
    printf("Type: %s,\n",
  ...
  case AST_ARGUMENT_LIST:
    ...
    // Adicione casos para outros tipos de nó conforme necessário
  ...
  // Fecha parênteses e imprime nova linha
  printf(")");

  // Se não for o último nó, imprime vírgula
  // Isso vai depender de como você está estruturando a impressão dos nós filhos
}
...
const char *get_token_type_name(token_types_t type) {
  ...
  case TOKEN_RBRACKET:
    return "]";
  // Adicione outros tipos de tokens conforme necessário
  default:
